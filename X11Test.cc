...
int main() {
    ...
    attributes.event_mask = SubstructureNotifyMask | KeyPressMask | KeyReleaseMask;
    ...
    x_set_size_hint(display, &window, {100, 100}, {1280, 720});
    ...
    window_buffer.init(window_size.y, window_size.x);
    XImage* x_window_buffer = XCreateImage(display, visual_info.visual, visual_info.depth, ZPixmap, 0, (char*)window_buffer.buffer, window_size.x, window_size.y, sizeof(u32) * 8, 0);
    ...
    while (is_running) 
    {
        ...
        for (u32 i = 0u; i < window_size.y; i++) {
            for (u32 j = 0u; j < window_size.y; j++) {
